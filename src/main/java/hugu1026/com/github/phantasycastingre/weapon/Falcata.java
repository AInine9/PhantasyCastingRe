...
public class Falcata extends Weapon {
    ...
    public Falcata(int original_power, int melting, int original_sharpness, int polishing) {
    ...
    protected int PowerDefendCalc(int original_power, int melting) {
        ...
        if (melting <= 0 || melting >= 1500) {
            ...
            power_defend = (int) (-20 + 0.08 * melting + -5.33e-05 * Math.pow(melting, 2)) + original_power;
        }
    ...
    protected int SharpDurabilityCalc(int original_sharpness, int polishing) {
        ...
        if (polishing <= 0 || polishing >= 1000) {
            ...
            sharp_durability = (int) (-7.11e-15 + 0.12 * polishing + -1.2e-04 * Math.pow(polishing, 2)) + original_sharpness;
        }
